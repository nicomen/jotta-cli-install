...
the leg never reached that phase because an earlier one failed. The counter-
check (see below) counts as part of "install": it's an install-time property
being tested, and its result is why Tumbleweed can show `❌/✅` — it installs
and runs fine, only the wrong-key rejection is broken.
